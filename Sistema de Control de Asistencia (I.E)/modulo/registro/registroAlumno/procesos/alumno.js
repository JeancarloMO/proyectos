...
function controlAdd(isNot){
    ...
    $("#convalidacion").prop("disabled", isNot);
...
function modalMulti(des){
    ...
    switch(des){
        case 1:
            ...
            $("#eventoTotal").attr('onclick', 'registrarAlumno();');
            break;
        case 2:
...
function registrarAlumno(){
    ...
    datosRegistro[4] = $("#convalidacion").val();
    ...
    if(datosRegistro[0] == "" || datosRegistro[1] == "" || datosRegistro[2] == "" || datosRegistro[3] == "" ){
        ...
        $.post("procesos/registrarAlumno.php", {valor: datosRegistro}, function(mensaje) {
            if(mensaje == "OK"){
                modalOKI(2);
                inicio();
                lista();
...
function editar(dato){
    $.post("procesos/editarDatos.php", {valor: dato}, function(mensaje) {
        ...
        document.getElementById("convalidacion").value = cadena[4];
...
function modificarAlumno(){
    ...
    if(datosRegistro[0] == "" || datosRegistro[1] == "" || datosRegistro[2] == "" || datosRegistro[3] == "" ){
        ...
        $.post("procesos/modificarAlumno.php", {valor: datosRegistro}, function(mensaje) {
            if(mensaje == "OK"){
                modalOKI(3);
                inicio();
                lista();
...
function lista(){
    $.post("procesos/listaAlumno.php", function(mensaje) {
        $("#contenidoAlumno").html(mensaje);
    });
}
